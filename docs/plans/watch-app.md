...
- Phase 5: Implemented — WatchGlyphView (DesignSystem), TrackRowView WatchGlyph integration with voiceover, TrackContextMenu watch items (Download to Watch / Remove from Watch / Retry), PlaylistDetailView Menu with Download All / Download All to Watch / Remove All from Watch, SourceDetailView same menu additions, WatchSettingsView (status header, on-watch stats, transfer queue, Re-send Catalog, Remove All), TransferPill in GlassDock, PhoneWatchSessionAdapter wrapping WCSession, AppState watch support (glyph state cache, download/remove actions, watch transfer count). Fixed WatchCatalog.swift + WatchLibraryFilter.swift compilation by excluding WatchSync/WatchPlayback from app target in project.yml. swift test: 681 tests, 0 failures.
- Phase 6: Implemented — WatchRootView (W1/W2) with Now Playing chip, navigation to playlists/albums/songs/storage. WatchComponents (WatchTrackRow, WatchCollectionRow). WatchPlaylistsView (W3) with list + empty state. WatchPlaylistDetailView (W4) with Play All + Shuffle buttons. WatchAlbumsView (W5/W6) with 32pt gradient+symbol artwork, Play All + Shuffle. WatchSongsView (W7) alphabetical, 5000-row cap. WatchStorageView (W11/W12) with usage header, per-entry size, swipe-delete, Remove All confirm. WatchEmptyStateView (W13) reusable. WatchSessionAdapter (WCSession delegate on watch side). WatchFixtureSeeder for smoke testing. WatchPlayer wrapper (queues, playback state). Added Album: Hashable conformance. All Core tests pass (681). Watch target builds.
- Phase 7: Implemented — WatchNowPlayingView (W8) with transport, progress bar, volume slider, Digital Crown volume control, Now Playing chip integration. WatchUpNextView (W9) tap-to-jump queue. WatchFetchOverlay (W10) progress + Cancel with WCSession fetch request/cancel wiring. AVPlayerOutput (WatchAudioOutput conformance) with AVAudioSession .longFormAudio activation. WatchPlayer rewritten with WatchPlayerEngine state machine + AVPlayer audio output + MPNowPlayingInfoCenter/MPRemoteCommandCenter integration + WatchPositionStore position persistence (save on pause/track-change/every-10s, restore on app launch). WatchSessionAdapter extended with sendFetchRequest/sendCancelFetch.
- Phase 8: WatchUITests target created in project.yml with WatchSmokeUITests.swift smoke test (testWatchAppBootsPlaysAndBrowses) using SEED_WATCH_FIXTURES. Smoke test stays out of CI. Build verified: iOS + watchOS both clean, 681 unit tests green. project.yml regenerated.
