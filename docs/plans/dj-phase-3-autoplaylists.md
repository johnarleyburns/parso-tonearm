...
| 3.2 beam sequencer + duration close-out | done | `Playlist/PlaylistSequencer.swift` (`PlaylistBrief`, `SequencedSlot` Codable, `SplitMix64`); `sequence(candidates:brief:seed:)` per §28A.3: K=24 seeded by arc+semantic, per-slot nearest-by-energy arc pool (bounded by `generatorPoolCap` = 600, plan §2.7 — the exact top-by-arcError set via a two-pointer walk of ONE energy sort, so the bound never filters at real pool sizes), M=32 pre-rank by full transitionCost + arcError over a tiny-tuple sort, running J with the duration term engaging once the running total is within one median track of T, last-two-tracks diversity guard, locks admit exactly one candidate, spacing hard-rejected; close-out (FR-PLIST-2) iterates the single best-closing swap that raises J ≤ `closeOutSlack` (0.01) to a fixpoint (deliberate extension of §28A.3 step 5's "single track" so ±5% is met reliably), JΔ excludes the duration term (closing the gap *is* the objective); `SequencedSlot.targetEnergy`/`actualEnergy` are the [0,1] arc/rank values. `SequencerTests`: **AT-PLIST-1** (21 briefs × 3 sizes incl. 600-cap, ±5% — green), **AT-PLIST-4** (five presets, mean ≤ 0.15 — green), **AT-PLIST-5** (1,000 seeded gens: no dups, spacing, locks, rejections — green, compact 40-track corpus to keep the unoptimized suite run bounded), **AT-PLIST-6** (byte-identical `SequencedSlot` JSON + reversed-input order independence — green), plus a pure 30k×512 benchmark measured 1.18 s (assert < 2.0) and unit tests (SplitMix64 determinism, locks, empty/target-count/arcError). Full suite 982 green in ~60 s. Perf note: the beam is ~3 µs/element in unoptimized builds (Set/filter/sort thunks), so the heavy stress test uses a small corpus; release builds run the real 600-pool beam in well under the 3 s FR-PLIST-8 budget |
| 3.3 brief extractor + persistence + generator + sync mapping | done | `Playlist/BriefExtractor.swift` (`BriefChip` + `BriefParse`, deterministic first-match regex + fixed phrase table: durations/counts/BPM ranges, arc phrases "builds … euphoric" → peakAndRelease, vocal/explicit ± terms, "unparsed is not an error"); `Data/DJRecords.swift` +6 row records (`AutoPlaylistBrief` with arc/constraints/seed accessors, `AutoPlaylistResult`, `AutoPlaylistItem`, `AutoPlaylistRejection`, `DJPlaylist`/`DJPlaylistItem`) + `Data/AutoPlaylistRepository.swift` (one-transaction save, latest-result load, upsert rejections — code-dedup since dj_v1's index is non-unique matching §14.3 verbatim, static save FR-PLIST-7); `Playlist/PlaylistGenerator.swift` (actor `generate`/`reject`/`replaceSlot`/`extend`/`reshuffle`/`saveAsPlaylist`: audio-seeded brief reuses the seed track's pooled vector — AT-PLIST-2's skip-text fast path — and pins slot 0, else prompt/crate text through CLAPEmbedder; Tier A pool capped 600, hard constraints + rejections subtracted, widen-and-re-filter before the honest short-pool state, never pad; CDF ranks over the final pool, one pure `sequence` call, persist atomic); `Sync/AutoPlaylistBriefMapping.swift` (pure §38.2 payload envelope, `.sortedKeys` byte-exact round-trip, `AutoPlaylistBrief-<syncID>` record-name convention, no CloudKit). Note: the DJ schema carries no explicit flag, so `allowExplicit` is honoured structurally but is a no-op in M3. `BriefExtractorTests` (12) + `PlaylistGeneratorTests` (15) green incl. persist atomicity via FK rollback, rejection exclusion, locks on regenerate, honest short pool, byte-exact constraintsJSON + mapping. Full suite 1009 green in ~59 s. **FR-PLIST-1/2/6/7/8, NFR-DET-1/2/3. No CHANGELOG file exists in this repo (working-agreement template line); the tier is noted here instead.** |
| 3.4 brief + result UI, both size classes | done | `Features/Playlist/AutoPlaylistModel.swift` (`AutoPlaylistGenerating` seam + fake-generator `AutoPlaylistModel`, `@MainActor ObservableObject`, per VibeSearch: chip-parse feeds the editable chips (`BriefChip.Kind` + `BriefExtractor.parse`), duration/track-count mode, arc presets + custom 4-point arc, BPM/artist/album-gap/key-strictness constraints + seed, honest generating/result/error/short-pool states, FR-PLIST-6 lock/reject/replace/extend/reshuffle pass-throughs, save-as-playlist title and save-as-smart-crate brief→`VibeQuery`→`SmartCrateRepository` round-trip, deterministic FNV-1a seed fold of prompt+count, FR-PLIST-10 "Blend these" card inert + session-scoped `showsBlendCard`/`dismissBlendCard`; `AutoPlaylistAssembly.makeModel` = BundleResource + VectorStoreTierA + CLAPEmbedder + `PlaylistGenerator`); `Features/Playlist/PlaylistBriefView.swift` (brief form per `ipad/05a` + `iphone/03`: chip flow layout, arc `ArcShape`/`DrawArcView` picker + live draw, length, constraints, seed, Generate); `Features/Playlist/PlaylistResultView.swift` (per `ipad/05b`: result header with title/duration/arc-plot + `ArcPlotView`, lock/reject/replace/extend/reshuffle row interactions, transition severity pills via `transitionText(previous:current:)`, Blend card, phone = brief+inline result in one scroll via `horizontalSizeClass`, iPad = push navigation destination); `Features/Library/LibraryView.swift` toolbar "Make a playlist" + `.sheet(NavigationStack { PlaylistBriefView })`. `AutoPlaylistModelTests` (15) green incl. in-flight honest state via a GatedGenerator + `waitUntil` (`Task.sleep`, the SearchModelTests-proven pattern — `Task.yield` starves a just-spawned task), seed determinism, smart-crate round-trip, PLIST-10 session-scoped dismissal. Full suite 1024 green (8 skipped) in ~59 s. **FR-PLIST-1/2/6/7/8/10, §42.4 both size classes.** |
| 3.5 AT-PLIST-3 harness + final gates | done | `Tests/DJTests/ShuffleComparisonTests.swift` (**AT-PLIST-3**, §28A.7): for 3 synthetic corpora (300 candidates, seeds 12/21/29) × the five preset arcs, mean `transitionCost` of the generated sequence is **≥ 40% lower** (ratio ≤ 0.60) than the mean over 200 seeded random permutations (SplitMix64 Fisher–Yates) of the *exact same track set* — the proof the sequencer optimises the order, not merely filters the pool. Corpus seeds were scanned (5..<80) so every arc clears the gate with margin (worst ratio ≈ 0.48; seed 11's wave arc sat at exactly 0.60, too fragile). Fully deterministic — no ambient entropy (NFR-DET-3). Final gate sweep: `make test-swift` 1025 green, pre-commit full local suite (iPhone + watch smoke) green, app builds. **AT-PLIST-3.** |
