...
| 2.3 recall@10 ≥ 0.95 gate | done | `RecallGateTests` only (no production changes): deterministic SplitMix64 corpus of 1,000 × 512-D unit vectors, 200 queries, int8 scan (real `Quantization`/`VectorMatrixScanner`) vs exact f32 cosine ground truth; **recall@10 = 0.9950 ≥ 0.95** (§16.6 gate passes, no f16 fallback needed); determinism + pinned error-bound tests green; 4 targeted tests green |
| 2.4 ranking+search service | done | `Semantic/Ranking.swift` (RankWeights 0.40/0.20/0.20/0.10/0.10, gaussian bpmFit σ=3, keyFit via `Camelot.compatibility` grades, energyFit 1−|Δ|/10, phraseFit bar-multiple, fusedScore, RankBreakdown, deterministic `HybridRankerOrdering.order` tie-break), `Semantic/SemanticSearchService.swift` (VibeQuery Codable, text search with +/− re-anchoring, audio-to-audio self-exclusion, honest coverage + `indexDidChange` warm, stated absence states); `CamelotKey` gained Codable + `init?(code:)`; `DJTrackEmbedding` Sendable; `DJTrackRepository.tracks(ids:)`; 20 targeted tests green |
| 2.5 Vibe Search UI+smart crates | done | `Features/VibeSearch/` (VibeSearchModel: `VibeSearching` seam, debounce 250 ms + in-flight cancel via generation guard, honest coverage, suggestion chips from the library's own descriptor distribution, stated model-not-downloaded state + ODR fetch, +/− refine, audio-to-audio, smart-crate save, privacy-once; VibeSearchView per mockups `ipad/04a-b`/`iphone/02` with per-row `RankBreakdown` decomposition, latency footer, Play·Queue·Save, locked "Load to deck"; `VibeSearchAssembly` production stack); `SmartCrate`/`CrateRule` records + `SmartCrateRepository` (canonical `.sortedKeys` queryJSON byte-exact round-trip, normalized bpm/camelot `crate_rule` rows, cascade delete, live `evaluate`); Library screen wired with "Find by feel" + per-row "More like this"; 16 targeted tests green (SearchModelTests 10, SmartCrateTests 6) |
