...
struct Biquad {
    ...
    static var identity: Biquad { Biquad() }

    mutating func reset() {
        x1 = [0, 0]; x2 = [0, 0]; y1 = [0, 0]; y2 = [0, 0]
    }

    mutating func process(_ x: Double, channel: Int) -> Double {
        let c = min(channel, 1)
...
struct EQEngine {
    ...
    init(sampleRate: Double = 48000, gains: [Double] = Array(repeating: 0, count: EQEngine.bandCount),
         bypassed: Bool = false) {
    ...
    mutating func setGains(_ newGains: [Double]) {
    ...
    mutating func process(_ sample: Double, channel: Int) -> Double {
    ...
    mutating func render(_ input: [Double], channel: Int = 0) -> [Double] {
